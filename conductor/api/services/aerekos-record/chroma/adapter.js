...
async function withRetry(reqFn, label = 'chroma', maxRetries = 2) {
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      const retriable = ['ECONNRESET', 'ETIMEDOUT', 'ENETUNREACH', 'EAI_AGAIN', 'ECONNREFUSED', 'EHOSTUNREACH'].includes(code) || 
...
function createChromaAdapter(connectionSettings = {}) {
  ...
  async function ensureCollection(collectionName = defaultCollection) {
    ...
    try {
      console.log('[chroma] ensureCollection list...')
      ...
      if (!found) {
        console.log('[chroma] creating collection', collectionName)
        await withRetry(() => 
  ...
  async function addEmbedding(id, embedding, metadata = {}, collectionName = defaultCollection) {
    ...
    console.log('[chroma] addEmbedding', { id, collectionId, metaKeys: Object.keys(cleanMeta || {}) })
    ...
    try {
      ...
      try {
        ...
        console.log('[chroma] addEmbedding retried with minimal metadata')
      } catch (e2) {
  ...
  async function querySimilar(embedding, topK = 5, filters = {}, collectionName = defaultCollection) {
    ...
    console.log('[chroma] querySimilar request', { collectionId, topK, filters })
    ...
    try {
      ...
      console.log('[chroma] querySimilar ok', { count: ids.length })
      ...
      if (e?.response?.status === 422 || e?.response?.status === 400) {
        console.warn('[chroma] 422 on query, response:', e.response?.data)
        
        // Fallback: query without where, then client-filter
        const fallback = { ...body }
        console.log('[chroma] fallback query (no where)')
  ...
  async function healthCheck() {
    try {
      await client.get(`${baseUrl}/api/v1/heartbeat`, { timeout: 5000 })
      return { status: 'healthy', url: baseUrl }
    } catch (e) {
      return { status: 'unhealthy', url: baseUrl, error: e.message }
    }
  }
