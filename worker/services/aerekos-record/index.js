...
const psqlAdapter = require('./psql/adapter')
const sqliteAdapter = require('./sqlite/adapter')
const elasticsearchAdapter = require('./elasticsearch/adapter')
const redisAdapter = require('./redis/adapter')
const chromaAdapter = require('./chroma/adapter')
...
const ADAPTERS = {
  ...
  psql: psqlAdapter,
  postgresql: psqlAdapter, // Alias
  postgres: psqlAdapter, // Alias
  sqlite: sqliteAdapter,
  elasticsearch: elasticsearchAdapter,
  es: elasticsearchAdapter, // Alias
  redis: redisAdapter,
}
...
const connect = (dbType, connectionSettings = {}) => {
  const adapter = ADAPTERS[dbType.toLowerCase()]
  
  if (!adapter) {
    throw new Error(`Unknown database type: ${dbType}. Supported types: ${Object.keys(ADAPTERS).join(', ')}`)
  }
